...
public abstract class InitialsProcessor implements IMatchProcessor<InitialsMatch> {
   ...
   * @param pInitPseudostateName the value of pattern parameter initPseudostateName in the currently processed match
   ...
   * 
   */
  public abstract void process(final Region pRegion, final Pseudostate pInitPseudostate, final String pInitPseudostateName, final Transition pInitTransition, final State pFirstState);
  
  @Override
  public void process(final InitialsMatch match) {
    process(match.getRegion(), match.getInitPseudostate(), match.getInitPseudostateName(), match.getInitTransition(), match.getFirstState());
