...
public class LocationConverter {
	...
	public List<Location> locationsFor(EObject modelElement, Class<? extends LocationQualifier> qualifier) {
		...
		if (null == symbols) {
			PluginLogger.logError(String.format("Unable to load debug symbols for class: %s", className));
			return locations;
		}

		ReferenceType type = classForName.get(className);
		if (null == type) {
			PluginLogger.logError(String.format("Reference type not found for class: %s", className));
			return locations;
		}
		...
		if (null == location) {
			PluginLogger.logError("Unable to resolve model element location");
			return locations;
		}
		...
		try {
			...
			PluginLogger.logError("Debug information is missing for location");
		}
