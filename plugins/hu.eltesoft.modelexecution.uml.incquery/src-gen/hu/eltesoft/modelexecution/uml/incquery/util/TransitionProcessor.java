...
public abstract class TransitionProcessor implements IMatchProcessor<TransitionMatch> {
   ...
   * 
   */
  public abstract void process(final Region pRegion, final State pSource, final Transition pTransition, final Event pEvent, final Signal pSignal, final State pTarget);
  
  @Override
  public void process(final TransitionMatch match) {
    process(match.getRegion(), match.getSource(), match.getTransition(), match.getEvent(), match.getSignal(), match.getTarget());
