...
public abstract class BehaviorParameterProcessor implements IMatchProcessor<BehaviorParameterMatch> {
   ...
   * 
   */
  public abstract void process(final Behavior pBehavior, final Parameter pParameter, final Type pType, final ParameterDirectionKind pDirection);
  
  @Override
  public void process(final BehaviorParameterMatch match) {
    process(match.getBehavior(), match.getParameter(), match.getType(), match.getDirection());
