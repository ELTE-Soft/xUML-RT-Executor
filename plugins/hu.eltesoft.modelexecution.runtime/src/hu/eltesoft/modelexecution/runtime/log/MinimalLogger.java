...
public class MinimalLogger implements Logger {
	...
	public MinimalLogger() {
		this.stateLogger = java.util.logging.Logger.getLogger(BaseRuntime.STATES_LOGGER_ID);
		this.transitionLogger = java.util.logging.Logger.getLogger(BaseRuntime.TRANSITIONS_LOGGER_ID);
		this.messageLogger = java.util.logging.Logger.getLogger(BaseRuntime.MESSAGES_LOGGER_ID);
	}

	@Override
	public void enterState(String state) {
		stateLogger.log(java.util.logging.Level.INFO, "Entered state {0}", state);
	}

	@Override
	public void exitState(String state) {
		stateLogger.log(java.util.logging.Level.INFO, "Exited state {0}", state);
	}

	@Override
	public void transition(String eventName, String messageName, String source, String target) {
		transitionLogger.log(java.util.logging.Level.INFO, "Transition by {1}( {0} ): {2} -> {3}",
